...
const Navbar = ({ theme, onToggleTheme }) => {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? "bg-slate-950/95 backdrop-blur-md shadow-lg py-4 border-b border-slate-900" : "bg-transparent py-6"}`}>
      <div className="container mx-auto px-6 md:px-12 flex justify-between items-center">
        <a href="#" className="text-2xl font-bold text-white tracking-tighter hover:text-cyan-400 transition-colors flex items-center gap-1">
        ...
        <div className="hidden lg:flex items-center space-x-6 text-sm font-medium text-slate-300">
          {navLinks.map((link) => (
            <a key={link.name} href={link.href} className="hover:text-cyan-400 transition-all hover:-translate-y-0.5 relative group">
              {link.name}
              <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-cyan-400 transition-all group-hover:w-full"></span>
            </a>
          ))}
          <a href={PERSONAL_INFO.resumeLink} className="px-5 py-2 rounded-full border border-cyan-400/50 text-cyan-400 hover:bg-cyan-400/10 hover:border-cyan-400 transition-all duration-300 shadow-md shadow-cyan-400/5">
            Resume.pdf
          </a>
          <button
        ...
        <div className="flex items-center gap-2 lg:hidden">
          ...
          <button className="text-slate-300 hover:text-white" onClick={() => setIsOpen(!isOpen)} aria-label="Toggle navigation menu">
      ...
      {isOpen && (
        <div className="lg:hidden absolute top-full left-0 w-full bg-slate-950 shadow-2xl border-b border-slate-900 py-6 flex flex-col items-center space-y-5">
          {navLinks.map((link) => (
            <a key={link.name} href={link.href} className="text-slate-300 hover:text-cyan-400 text-lg transition-colors w-full text-center py-2" onClick={() => setIsOpen(false)}>
              {link.name}
            </a>
          ))}
          <a href={PERSONAL_INFO.resumeLink} className="px-8 py-3 rounded-full bg-cyan-500/10 border border-cyan-400 text-cyan-400 hover:bg-cyan-400 hover:text-slate-950 transition-all text-center w-2/3" onClick={() => setIsOpen(false)}>
            Download Resume
          </a>
          <button onClick={onToggleTheme} className="inline-flex w-2/3 items-center justify-center gap-2 rounded-full border border-slate-700 px-8 py-3 text-slate-300 transition-colors hover:text-cyan-400">
            {theme === "dark" ? <SunIcon /> : <MoonIcon />}
